alert("Весь текст в комментариях...")

/* Задание 1: Для практикума из занятия 7 продумать, где можно применить замыкания.

Код из ДЗ для 7 урока я сдавала после 7 урока, не думаю, что могла бы там что-то серьезно изменить без подсказок и указаний, поэтому к этому уроку код приложен ниже только в виде комментария и выполнятся не будет (я убрала стартовые блоки из html документа, раскомментировать не поможет).
Разбор ДЗ я пока не смотрела, если у Вас есть какие-то комментарии по коду, буду рада услышать.

По поводу соображений о замыканиях:

--- В интернет-магазине

Я не уверена, что все правильно понимаю, но часть прослушек (EventListener'ов) у меня пользуется локальными переменными. Например, прослушка кнопки productsBasketSwitchButton, задаваемая в функции headerPrint(), использует локальные переменные этой функции: headerDiv, basketDiv, productDiv. То есть она знает об этих переменных даже после того как headerPrint() отрисовала верхнюю часть страницы и закончила свое исполнение. Получается, заданная анонимная функция замкнута на локальные переменные headerPrint()?

Теоретически, функцию переключения картинок можно было бы реализовать через замыкания. Задать общую функцию, в ней ввести переменную-счетчик, на этой переменной основать переключение картинок и вернуть его. И потом запустить внешнюю функцию для каждой картинки отдельно.

Возможно, что-то похожее можно было бы реализовать и со сворачиванием блоков. Разбить функцию на две части: само сворачивание сделать замыканием и запустить для каждого блока отдельно, а переключение по кнопке далее реализовать отдельно с использованием замкнутых функций... Но этот пример я пока слабо представляю.

И у меня в программе есть две глобальные переменные: список продуктов и корзина. Насколько я поняла из написанного в методичке, функции, использующие глобальные переменные, замыканиями не являются (хотя и замкнуты на них). Но эти два объекта можно реализовать с помощью замыканий.
1. Корзина. Если необходимо реализовать магазин, в котором могут одновременно закупаться несколько пользователей, иметь одну корзину на всех нельзя. Можно задать корзину и функции работы с ней с помощью замыкания: создать внешнюю функцию, корзину определить внутри, а необходимые функции работы с корзиной возвращать. Тогда мы можем внешнюю функцию присвоить нескольким переменным и у каждой переменной появится своя корзина. Про свою они будут знать, а про соседние - нет.
2. Список продуктов. Если на сайте список продуктов/товаров один на всех, то смысла создавать замыкание я, если честно, не очень вижу. То есть понятно, что это можно сделать для защиты данных: внешние функции не будут иметь доступа к внутренним, но иных причин я пока не вижу. Зато если на сайте пользователи могут выставлять свои продукты (книги, например), то имеет смысл создать замыкание для списка продуктов аналогично корзине, чтобы каждый пользователь работал со своим списком.
С другой стороны, в реальности оно все через базы данных, наверное, реализуется...

 На этом мои идеи кончились...

--- В змейке и крестиках ноликах

Можно, например, состояние игры реализовать через замыкания: сделать локальной переменной внешней функции и все нужные функции из внешней возвращать. Тогда для каждой новой игры будет создаваться свое состояние (теоретически, так можно будет сохранять старые игры и доигрывать их позже, при этом имея возможность начать новые).

Аналогично, можно сделать размер доски и прочие параметры игры (скорость змейки, например) локальными переменными какой-то внешней функции. Тут смысл больше в том, чтобы не думать о повторении имен переменных (насколько я поняла из примера в методичке, могут быть проблемы при работе в команде из-за создания одинаковых имен), если на сайте много игр с досками.

 */

/*Задание 2

До сдачи я честно ничего не выполняла, поэтому буду признательна, если Вы прокомментируете, если я где-то ошиблась в рассуждениях.
...
alert(a);  -- Вернет 1? Я не очень разобралась с window (не помню его из лекций) и, соответственно, с первой строчкой, но по логике, мне кажется, как-то так должно быть. Если выполнять только этот кусок кода, то в window ничего быть не должно, значит, "a" in window должно вернуть false, тогда !("a" in window) должно вернуть true и получается, будет создана переменная a = 1 и должно будет вывестись ее значение.
...
alert(a);   -- Выведет null? Насколько я понимаю, про функцию а(х) знает только тело функции а(х), а остальная программа про такую переменную не в курсе, поэтому вернет то, что переменной не существует.
...
var a;
alert(a);   -- по идее, должен вывести undefined, так как объявление переменной было после объявления функции, и теперь а, хоть и существует, но не задано.
...
b(1, 2, 3);   -- Должен вывести 3, так как alert(a) примет значение a = 3, а alert() выполнится перед всем остальным в теле функции так же, как делает prompt() (как это было в крестиках-ноликах и змейке). Потом уже, так как массив arguments не задан, браузер должен выдать ошибку, потому что там попытка изменить несуществующий объект, а он так не умеет.
...
a.call(null);   -- Должен вывести null. метод call() позволяет задать значение this. Получается, при таком вызове функции this будет равно null и, соответственно, на экран выведется null.
